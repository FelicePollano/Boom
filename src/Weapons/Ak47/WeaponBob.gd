...
func _process(delta):
	...
	if (Input.is_key_pressed(KEY_W) or Input.is_key_pressed(KEY_S)) and disabled == false:
		...
		offset.y = lerp(offset.y, initial_offset.y + (sine_function.y * intensity_y) + intensity_y, 0.1)
		offset.x = lerp(offset.x, initial_offset.y + (sine_function.x * intensity_x), 0.1)
	else:
		offset.y = lerp(offset.y, initial_offset.y, 0.1)
		offset.x = lerp(offset.x, initial_offset.x, 0.1)
